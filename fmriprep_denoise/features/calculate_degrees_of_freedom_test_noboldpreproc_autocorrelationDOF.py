...
from scripts.halfpipe_to_denoise.transition_timeseries_imputation import FEATURE_RENAME_MAP
...
def estimate_subject_dof_from_autocorrelation(time_series_2d):
    T, N = time_series_2d.shape
    dof_vals = []
    rho_vals = []
    ...
    return np.mean(dof_vals), np.mean(rho_vals)
...
def main():
    ...
    for strategy_name, parameters in benchmark_strategies.items():
        ...
        for cf_path in confound_files:
            ...
            # --- New: Estimate DOF lost due to Gaussian high-pass filtering via autocorrelation ---
            subject_id = Path(cf_path).name.split("_")[0]  # e.g., 'sub-pixar001'

            # Map strategy name to its HALFpipe filename prefix
            REVERSE_FEATURE_RENAME_MAP = {v: k for k, v in FEATURE_RENAME_MAP.items()}
            feature_key = REVERSE_FEATURE_RENAME_MAP.get(strategy_name)

            if feature_key is None:
                logging.warning(f"[{strategy_name}] Strategy not found in FEATURE_RENAME_MAP — skipping high-pass DOF estimate.")
                dof_lost_from_hp = np.nan
            else:
                # Build the path to the correct timeseries file
                # Derive correct base path to HALFpipe outputs (replace fmriprep with halfpipe)
                halfpipe_path = Path(args.fmriprep_path).parent / "halfpipe"

                # Now build the correct path
                time_series_path = halfpipe_path / subject_id / "func" / \
                    f"{subject_id}_{specifier}_feature-{feature_key}_atlas-schaefer400_timeseries.tsv"

                if time_series_path.is_file():
                    try:
                        ts_df = pd.read_csv(time_series_path, sep="\t")
                        time_series = ts_df.to_numpy()
                        dof_eff, _ = estimate_subject_dof_from_autocorrelation(time_series)
                        dof_lost_from_hp = full_length - dof_eff
                    except Exception as e:
                        logging.warning(f"[{strategy_name}] Failed to process timeseries file {time_series_path}: {e}")
                        dof_lost_from_hp = np.nan
                else:
                    logging.warning(f"[{strategy_name}] Timeseries file not found: {time_series_path}")
                    dof_lost_from_hp = np.nan

            # Store in the stats dict under the appropriate column
            stats[(strategy_name, "high_pass")] = dof_lost_from_hp
